...
/settings
创建临时页面组件，例如：
Dashboard.vue、Planner.vue，页面内容仅展示页面名称。
验收标准
所有菜单点击可跳转至对应页面。
Task 5：基础组件系统
目标
...
每次让 AI 只处理一个 Task。
修改说明（我做的格式修复点）
统一 Markdown 标题层级，原文档标题混乱，把大章节用 ##，子任务用 ###，符合 PRD 文档同一份风格；
所有代码块、bash 脚本、ts 代码、目录树全部加上标准 ``` 代码块标记；
列表、✅❌ 统一格式；
移除无效 “运行” 标记，修复代码块语法错误；
流程箭头、调用链路使用代码块格式化；
增加分隔，阅读体验和你给的 PRD 文档风格对齐；
增加小标题、加粗区分目标 / 操作 / 输出 / 验收标准。
需要我把原始 PRD 文档 + 这份 Task Breakdown合并成一份完整仓库文档包（直接可复制到仓库 markdown）吗？
不需要合并，单独生成一份 Task Breakdown 完整仓库文档包（直接可复制到仓库 markdown）
Task Breakdown
版本：v1.0
状态：开发执行文档
目标：指导 Personal OS 第一阶段开发
...
/entertainment
/settings
创建临时页面组件，例如：Dashboard.vue、Planner.vue，页面内容仅展示页面名称。
验收标准
所有菜单点击可跳转至对应页面。
Task 5：基础组件系统
目标
